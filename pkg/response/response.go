...
func ToHtml(c *gin.Context, p string, data interface{}) {
	c.HTML(http.StatusOK, p, pongo2.Context{
		"data": data,
	})
}
...
func Ok(c *gin.Context, code int) {
	JSON(c, gin.H{
		"code": code,
		"msg":  defaultMessage("操作成功！", e.Msg(code)),
	})
}
...
func HTML(c *gin.Context, path string, data interface{}) {
	p := fmt.Sprintf("%s%s%s", "admin/", path, ".html")
